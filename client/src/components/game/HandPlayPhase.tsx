...
export default function HandPlayPhase() {
  ...
  if (isTrickComplete) {
    return (
      <motion.div
        ...
        exit={{ opacity: 0, y: 10 }}
        className="fixed bottom-32 sm:bottom-24 left-1/2 transform -translate-x-1/2 z-40"
      >
        <Button
          onClick={nextTrick}
          className="bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-400 hover:to-blue-500 text-white font-bold h-14 px-8 touch-manipulation shadow-xl flex items-center gap-2 text-lg"
        >
          Next Trick
  ...
  // Local player's turn - show play button or select prompt
      ...
      <AnimatePresence mode="wait">
        {selectedCard ? (
          <motion.div
            ...
            className="fixed bottom-6 sm:bottom-4 left-1/2 transform -translate-x-1/2 z-40 max-w-full px-4"
          >
            <Button
              onClick={handleCardPlay}
              className="w-full max-w-sm mx-auto bg-gradient-to-r from-green-500 to-green-600 hover:from-green-400 hover:to-green-500 text-white font-bold h-14 touch-manipulation shadow-xl flex items-center justify-center gap-3 text-lg"
            >
          ...
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: 10 }}
            className="fixed bottom-6 sm:bottom-4 left-1/2 transform -translate-x-1/2 z-40"
          >
            <div className="bg-gray-900/90 backdrop-blur-sm text-white px-4 py-2 rounded-full shadow-lg flex items-center gap-2 border border-gray-700">
              <Hand className="h-4 w-4 text-blue-400" />
              <span className="text-sm font-medium">Tap a card to select</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
